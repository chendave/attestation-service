...
impl Verifier for CCA {
    async fn evaluate(
        ...
        let result = match cr.run(n, my_evidence_builder, token.clone()).await {
            Err(e) => {
                log::error!("Error: {}", e);
                panic!("Attestation failed with error: {:?}", e);
            }
            Ok(attestation_result) => attestation_result,
        };

        // Get back the pub key to decrypt the ear which holds raw evidence and the nonce
        let public_key_pem = verification_api.ear_verification_key_as_pem()?;
        let dk = jwt::DecodingKey::from_ec_pem(public_key_pem.as_bytes()).unwrap();
        let plain_ear = Ear::from_jwt(result.as_str(), jwt::Algorithm::ES256, &dk).unwrap();
        println!("EAR nonce: {:?}", plain_ear.nonce);
        println!("EAR evidence: {:?}", plain_ear.raw_evidence);

        let ear_nonce = plain_ear.nonce.unwrap();
        let nonce_byte = engine::GeneralPurpose::new(&alphabet::URL_SAFE, general_purpose::PAD)
            .decode(ear_nonce.to_string())
            .unwrap();

        //let s = String::from_utf8(hash_of_nonce_pubkey).expect("Found invalid UTF-8");
        if hash_of_nonce_pubkey != nonce_byte {
            return Err(anyhow!(
                "HASH(nonce||pubkey) is different from that in CCA token"
            ));
        }
